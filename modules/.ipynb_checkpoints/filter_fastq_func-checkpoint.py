def check_params(**kwargs):   # Функция проверяет введены ли параметры фильтрации. Если нет, то задаёт значения по умолчанию.
...
def gc_comp_calc(seq):
    return (seq.count('G')+seq.count('C'))/len(seq)*100


def quality_calc(seq_q):
    quality = 0
